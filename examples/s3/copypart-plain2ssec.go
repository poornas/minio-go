...
func main() {
	...
	//ssec
	srcInfo, err := s3Client.StatObject(bucketName, objectName, minio.StatObjectOptions{minio.GetObjectOptions{ServerSideEncryption: nil}})
	if err != nil {
	...
	uploadID, err := s3Client.NewMultipartUpload(destBucketName, destObjectName, minio.PutObjectOptions{ServerSideEncryption: dstencryption})
	if err != nil {
	...
	if err != nil {
		fmt.Println("fspart:", err)
	}
	fmt.Println("fspart:", fstPart)
	...
	fmt.Println(sndPart, "secondpart:")
	...
	fmt.Println("fspart:", fstPart, "scpart:", sndPart, "thdpart:", lstPart)
	...
	fmt.Println("completed upl;oads..")
